...
from lwin_matcher.app import create_app
...
try:
    ...
    # Fallback to alternative method names if get_all doesn't exist
    _alt_methods = [
        "get_all_lwin",
        "fetch_all",
        "get_lwin_table",
    ...
    lwin_table = None
    for _m in _alt_methods:
        if hasattr(lwin_database_client, _m):
            lwin_table = getattr(lwin_database_client, _m)()
            break
    if lwin_table is None:
        raise
...
# CSV field order (keep EXACT order as required)
CSV_FIELD_ORDER = [
    # lot info
    "lot_external_id",
    "lot_name",
