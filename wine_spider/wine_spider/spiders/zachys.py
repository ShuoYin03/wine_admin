                ...
                for auction_data in auction_datas:
                    ...
                    yield scrapy.Request(
                        ...
                        meta={
                            ...
                            "catalog_id": auction_data.get("id"),
                            "auction_seo_name": auction_data.get("auction_seo_url")
                        },
                        dont_filter=True
                    )
    ...
    async def get_excel_and_info(self, response):
        ...
        try:
            ...
            for lot in soup.find_all("div", class_="list-cols info-col"):
                lot_item = LotItem()
                lot_id = lot.select_one("a.auc-lot-link").get_text(strip=True).strip().split("#")[-1]
                lot_item['external_id'] = f"{auction_id}_{lot_id}"
                lot_item['auction_id'] = auction_id
                currency = lot.select_one("span[class^=scur]").get_text(strip=True).strip()
