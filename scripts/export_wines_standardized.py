...
def main() -> None:
    ...
    out_dir = os.path.join(os.path.dirname(__file__), "export_wines_standardized")
    ...
    with psycopg2.connect(**conn_params) as conn:
        ...
        lwin_matching = pd.read_sql_query(
            """
            SELECT
                lm.lot_id AS lot_item_id,
                lm.lwin,
                lm.lwin_11
            FROM lwin_matching lm
            """,
            conn,
        )
    ...
    df = df.merge(lwin_matching, on="lot_item_id", how="left")
    ...
    for wine_name, patterns in WINES.items():
        ...
        matched = matched.drop(columns=["lot_name_std", "lot_producer_std"], errors="ignore")
        ...
        lot_items_count = len(matched)
        lots_count = matched["lot_external_id"].nunique(dropna=True)
        total_rows += lot_items_count
        total_lot_ids.update(matched["lot_external_id"].dropna().astype(str).tolist())

        print(
            "Exported: "
            f"{wine_name} -> {out_path} | lots={lots_count} | lot_items={lot_items_count}"
        )

    print(
        "Totals: "
        f"lots={len(total_lot_ids)} | lot_items={total_rows}"
    )
